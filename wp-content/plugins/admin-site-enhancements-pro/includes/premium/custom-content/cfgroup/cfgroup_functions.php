...
function get_cf_time( $raw_time, $frontend_display_format ) {
    if ( ! empty( $raw_time ) && is_numeric( str_replace( ':', '', $raw_time ) ) ) {
        $time_value = explode(":", $raw_time );
        ...
        // Get am / pm
        $ampm = ( $hour < 12 ? 'am' : 'pm');
        switch ( $frontend_display_format ) {
            case 'g:i A':
            case 'h:i A':
        ...
        switch ( $frontend_display_format ) {
            ...
            case 'G:i':
                // Do nothing, $hour is already in 24 hour format without leading 0
                break;
        ...
        return $raw_time; // Already formatted
    }
