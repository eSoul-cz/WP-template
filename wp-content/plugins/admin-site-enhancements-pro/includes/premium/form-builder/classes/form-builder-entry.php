            ...
            foreach ( $data['item_meta'] as $field_id => $field_value ) {
                if ( is_array( $field_value ) && ! empty( $field_value ) ) {
                    foreach ( $field_value as $key => $value ) {
                        // Matrix of Uniform Dropdowns field
            ...
            foreach ( $values['item_meta'] as $field_id => $meta_value ) {
                ...
                if ( isset( $meta_value['likert_rows'] ) ) {
                    ...
                    foreach ( $meta_value['likert_rows'] as $key => $value ) {
                        ...
                        $processed_meta_value['likert_rows'][$key]['row_label'] = $row_label;
                        $processed_meta_value['likert_rows'][$key]['column_label'] = $column_label;
                    }
                    ...
                    foreach ( $meta_value['dropdown_matrix_rows'] as $key => $value ) {
                        foreach ( $value as $val_key => $val ) {
                            if ( ! empty( $val ) ) {
                                ...
                                $processed_meta_value['dropdown_matrix_rows'][$key]['row_label'] = $row_label;
                                $processed_meta_value['dropdown_matrix_rows'][$key]['choices'][$val_key]['column_label'] = $column_label;
                                $processed_meta_value['dropdown_matrix_rows'][$key]['choices'][$val_key]['selected_option'] = $selected_option;
                            }
                        }
                    }
                ...
                if ( isset( $meta_value['dropdowns_matrix_rows'] ) ) {
                    ...
                    foreach ( $meta_value['dropdowns_matrix_rows'] as $key => $value ) {
                        foreach ( $value as $val_key => $val ) {
                            ...
                            $processed_meta_value['dropdowns_matrix_rows'][$key]['row_label'] = $row_label;
                            $processed_meta_value['dropdowns_matrix_rows'][$key]['choices'][$val_key]['column_label'] = $column_label;
                            $processed_meta_value['dropdowns_matrix_rows'][$key]['choices'][$val_key]['selected_option'] = $selected_option;
                        }
                    }
